...
const Notification = ({
  ...
  return (
    <div
      className={`fixed top-4 right-4 z-50 animate-in slide-in-from-right-4 duration-500`}
    >
      <div
        className={`${style.bg} ${style.border} border-l-4 text-white px-6 py-4 rounded-lg shadow-lg flex items-center space-x-3 min-w-[350px] max-w-md`}
      >
...
// Ticket Service
class TicketService {
  static async createTicket(ticketData: Partial<Ticket>): Promise<{
    success: boolean;
    ticketId: string;
    ticket: Ticket;
    message: string;
...
export function UserReportsPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (user.communityId) {
        const communityTickets = await TicketService.getCommunityTickets(
          user.communityId
        );
        setCommunityTickets(communityTickets);
        console.log('Community tickets loaded:', communityTickets);
        // Log specific technician data for debugging
        communityTickets.forEach(ticket => {
          if (ticket.assignedTo) {
            console.log(`Ticket ${ticket.id}:`, {
              assignedTo: ticket.assignedTo,
              assignedTechnicianName: ticket.assignedTechnician?.name,
              aiRecommendedTechnicianName: ticket.aiMetadata?.recommendedTechnician?.name,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const ticketData: Partial<Ticket> = {
        title:
          formData.title || generateTitleFromDescription(formData.description),
      ...
      if (result.success) {
        ...
        showNotification(
          "success",
          result.message || "Your issue has been submitted successfully!"
        );
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground text-lg">
                Welcome back, {user?.name}! Manage your tickets and community
                reports.
              </p>
            </div>
            ...
            <Card className="shadow-lg border-border">
              <CardHeader>
                ...
                <p className="text-muted-foreground">
                  Describe the problem and upload photos to help technicians
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="space-y-2">
                    <div className="flex ">
                      ...
                      <img src="/mic.png" className="w-7 border border-black rounded-full p-1 mx-2"></img>
                    </div>
                    <Textarea
                    ...
                    <CardContent className="pt-6">
                      <div className="flex flex-col lg:flex-row gap-6">
                          ...
                          <div className="w-full lg:w-48 h-48 flex-shrink-0">
                            <img
                              src={
                                ...
                                (ticket.imageUrl && ticket.imageUrl[0]) ||
                                ""
                              }
                              alt="Issue"
                            ...
                            {((ticket.images && ticket.images.length > 1) ||
                              ...
                              <p className="text-xs text-muted-foreground mt-1 text-center">
                                ...
                                {((ticket.images && ticket.images.length) ||
                                  (ticket.imageUrl && ticket.imageUrl.length) ||
                                  1) - 1}{" "}
                                more image
                                {((ticket.images && ticket.images.length) ||
                                  (ticket.imageUrl && ticket.imageUrl.length) ||
                        ...
                        <div className="flex-1 space-y-4">
                                ...
                                <div className="flex-1">
                                  ...
                                  <p className="text-sm text-green-800 dark:text-green-200 font-medium">
                                    {ticket.assignedTechnician?.name ||
                                      ticket?.aiMetadata?.recommendedTechnician
                                        ?.name ||
                                      "Technician Assigned"}
                                  </p>
                                  {ticket.assignedTechnician?.expertise &&
                                    ticket.assignedTechnician.expertise.length >
                                      0 && (
                                      <p className="text-xs text-green-600 dark:text-green-400 mt-1">
                                        Expertise:{" "}
                                        {ticket.assignedTechnician.expertise.join(
                                          ", "
                                        )}
                                      </p>
                                    )}
                          ...
                          {ticket.status === "spam" && ticket.spamMetadata && (
                              ...
                              <div className="flex items-start space-x-3">
                                ...
                                <div className="flex-1">
                                  ...
                                  <p className="text-xs text-red-600 dark:text-red-400 mb-2">
                                    {ticket.spamMetadata.reason}
                                  </p>
                                  <div className="flex gap-2">
                                    <Badge className="bg-red-100 text-red-800 border-red-300 text-xs">
                                      {Math.round(
                                        (ticket.spamMetadata.confidence || 0) *
                                          100
                                      )}
                                      % confidence
                                    </Badge>
                                    <Badge className="bg-gray-100 text-gray-800 border-gray-300 text-xs">
                                      Detected:{" "}
                                      {formatDate(
                                        ticket.spamMetadata.detectedAt
                                      )}
                                    </Badge>
                                  </div>
                                </div>
                          ...
                          {/* AI Insights */}
                          {ticket.aiMetadata && ticket.status !== "spam" && (
                                  ...
                                  <div className="space-y-2 text-sm text-blue-800 dark:text-blue-200">
                                    <div className="flex flex-wrap gap-2">
                                      <Badge className="bg-blue-100 text-blue-800 border-blue-300 text-xs">
                                        Category:{" "}
                                        {ticket.aiMetadata.predictedCategory}
                                      </Badge>
                                      <Badge
                                        className={`text-xs border ${
                                          ticket.aiMetadata.predictedUrgency ===
                                          "high"
                                      ...
                                      {ticket.aiMetadata.confidence && (
                                        <Badge className="bg-gray-100 text-gray-800 border-gray-300 text-xs">
                                          {Math.round(
                                            ticket.aiMetadata.confidence * 100
                                          )}
                                          % confidence
                                        </Badge>
                                      )}
                                    ...
                                    {ticket.aiMetadata.recommendedTechnician
                                      ...
                                      <p className="text-xs text-blue-600 dark:text-blue-400 mt-2 italic">
                                        ...
                                        {
                                          ticket.aiMetadata
                                            .recommendedTechnician.reasoning
                                        }
                                        "
                                      </p>
                                    )}
                          ...
                          {/* Tools & Materials */}
                          {(ticket.requiredTools?.length ||
                            ticket.requiredMaterials?.length) && (
                            <div className="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-4 border border-gray-200 dark:border-gray-800">
                              <div className="flex items-start space-x-3">
                                ...
                                <div className="flex-1">
                                    ...
                                    ticket.requiredTools.length > 0 && (
                                      <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                          {ticket.requiredTools.map(
                                            (tool, index) => (
                                              <div
                                                key={index}
                                                className="bg-blue-50 dark:bg-blue-950/30 rounded-md px-3 py-2 border border-blue-200 dark:border-blue-800"
                                              >
                                                <div className="flex items-center justify-between">
                                  ...
                                  {ticket.requiredMaterials &&
                                    ticket.requiredMaterials.length > 0 && (
                                      <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
                                        <p className="text-xs font-medium text-gray-600 dark:text-gray-400 mb-2">
                                          Required Materials:
                                        </p>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                          {ticket.requiredMaterials.map(
                                            (material, index) => (
                                              <div
                                                key={index}
                                                className="bg-green-50 dark:bg-green-950/30 rounded-md px-3 py-2 border border-green-200 dark:border-green-800"
                                              >
                                                <div className="flex items-center justify-between">
